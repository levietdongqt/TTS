...
public class admRefBoService implements IAdmRefBoService {
  ...
  public void buildBoFieldDir(List<ADM_REF_BO> root) {
    ...
    try {
      for (ADM_REF_BO item : root) {
        ...
        if (parent != null) {
          if (item.getBoFieldName().trim().isEmpty()) {
            item.setBoFieldDir(parent.getBoFieldDir());
            item.setLevel(parent.getLevel() + 1);
          } else if (parent.getBoFieldDir().isEmpty()) {
  ...
  public List<ADM_REF_BO> mappingUiCtrlCodeToRefBo(List<ADM_REF_BO> hasUiCtrlCodeList,
    ...
    for (ADM_REF_BO adm_REF_BO : hasUiCtrlCodeList) {
      if (adm_REF_BO.getBoFieldName().isEmpty()) {
        continue;
      }
  ...
  public List<ADM_REF_BO> getListByBoFieldDir(TreeSet<ADM_REF_BO> boFieldDir) {
    String JSON = "";
    try {
      JSON = new ObjectMapper().writeValueAsString(boFieldDir);
      System.out.println(JSON);
    } catch (JsonProcessingException e) {
      e.printStackTrace();
    }
    return admRefBoRepo.getListByBoFieldDir(JSON);
  }
